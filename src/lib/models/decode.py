...
def _topk(scores, K=40):
    ...
    topk_scores, topk_inds = torch.topk(scores.view(batch, cat, -1), K)  # inter-class (1,80,128,128)->(1,80,40)

    topk_inds = topk_inds % (height * width)  # (1,80,40)
    topk_ys = (topk_inds / width).int().float()
    topk_xs = (topk_inds % width).int().float()

    topk_score, topk_ind = torch.topk(topk_scores.view(batch, -1), K)  # 类间(1,80*40)->(1,40)
    topk_clses = (topk_ind / K).int()
    topk_inds = _gather_feat(
        topk_inds.view(batch, -1, 1), topk_ind).view(batch, K)  # output(1,40)
...
def smk3d_decode(heat, rot, depth_offset, dim_offset, hm_offset=None, K=40):
    '''
    retrieve 3D Objects attributes: [rotation, projected_CenterPoint, dimension, classes, scores]
    '''
    batch, cat, height, width = heat.size()
    heat = _nms(heat)

    scores, inds, clses, ys, xs = _topk(heat, K=K)
    if hm_offset is not None:
        hm_offset = _transpose_and_gather_feat(hm_offset, inds)
        hm_offset = hm_offset.view(batch, K, 2)
        xs = xs.view(batch, K, 1) + hm_offset[:, :, 0:1]
        ys = ys.view(batch, K, 1) + hm_offset[:, :, 1:2]
    else:
    ...
    depth_offset = _transpose_and_gather_feat(depth_offset, inds)
    depth_offset = depth_offset.view(batch, K, 1)
    dim_offset = _transpose_and_gather_feat(dim_offset, inds)
    dim_offset = dim_offset.view(batch, K, 3)
    clses = clses.view(batch, K, 1).float()
    scores = scores.view(batch, K, 1)
    xs = xs.view(batch, K, 1)
    ys = ys.view(batch, K, 1)

    detections = torch.cat([xs, ys, scores, rot, depth_offset, dim_offset, clses], dim=2)
...
def ttf_ctdet_decode(heat, wh, reg=None, cat_spec_wh=False, K=100):
    batch, cat, height, width = heat.size()
    heat = _nms(heat)

    # get topK score boxes indices
    scores, inds, clses, ys, xs = _topk(heat, K=K)
    if reg is not None:
        reg = _transpose_and_gather_feat(reg, inds)
        reg = reg.view(batch, K, 2)
