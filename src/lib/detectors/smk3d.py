...
class Smk3dDetector(BaseDetector):
    def __init__(self, opt):
        super(Smk3dDetector, self).__init__(opt)
    ...
    def process(self, images, return_time=False, meta=None):
        '''
        parse the attribute of objects: [location, scores, orientation, dimension, class]
        and encode the form of [scores, cls , corners_3d]
        '''
        with torch.no_grad():
            torch.cuda.synchronize()
            output = self.model(images)[-1]
            ## parse the attributes of the 3d objects by the network regression
            output['hm'] = output['hm'].sigmoid_()
            torch.cuda.synchronize()
            forward_time = time.time()

            dets = smk3d_decode(output['hm'], output['rot'], output['dep'],
                                output['dim'], output['offset'], K=self.opt.K)
        if return_time:
